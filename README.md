...
The minimum length the modal accepts in random mode is the number of currently-enabled character classes (since each one contributes a guaranteed character): one class lets you go as low as 1. Because the "safe symbols" and "all symbols" sets share a single slot (enabling `5` supersedes `4`), the most distinct classes you can have active at once is four, so the minimum tops out at 4.
